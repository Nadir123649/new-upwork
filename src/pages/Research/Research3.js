...
const Research = () => {
  ...
  // New state and ref for dynamic height adjustment
  const researchAreaRef = useRef(null);
  const [researchAreaHeight, setResearchAreaHeight] = useState('auto');
  ...
  // New useEffect for dynamic height adjustment
  useEffect(() => {
    const updateResearchAreaHeight = () => {
      if (researchAreaRef.current) {
        const containerHeight = researchAreaRef.current.scrollHeight;
        const maxHeight = window.innerHeight * 0.7; // 70% of viewport height
        const newHeight = Math.min(containerHeight, maxHeight);
        setResearchAreaHeight(`${newHeight}px`);
      }
    };

    updateResearchAreaHeight();
    window.addEventListener('resize', updateResearchAreaHeight);

    return () => {
      window.removeEventListener('resize', updateResearchAreaHeight);
    };
  }, [messages]);
  ...
  const handleSubmit = async (e) => {
    ...
    console.log('Current research response count:', researchResponseCount);
    ...
    if (prompt.trim() && effectiveUserId) {
      ...
      try {
        ...
        if (!isAuthorized) {
          ...
          setResearchResponseCount(prevCount => {
            const newCount = prevCount + 1;
            console.log('New research response count:', newCount);
            return newCount;
          });
        }
  ...
  const samplePrompts = [
    { title: "Technology", text: "Tell me about Apple Comprehensive Stock" },
    { title: "Healthcare", text: "Provide Pfizer's Comprehensive Catholic Stock AI Info." },
    { title: "Financials", text: "Goldman Sachs Comprehensive Stock Scores." }
  ];
  ...
  return (
    <div className="research-container">
      <div 
        ...
        ref={researchAreaRef}
        style={{ height: researchAreaHeight, maxHeight: '70vh', overflowY: 'auto' }}
      >
