...
const Chat = () => {
  ...
  const renderMessageContent = (content) => {
    ...
    if (typeof content === 'object' && content.response && content.response.sections) {
      return (
        <div className="structured-analysis">
          {content.response.sections.map((section, index) => (
    ...
    if (typeof content === 'string') {
      ...
      if (cleanedContent.includes('<div class="ai-initiative-report">')) {
        ...
        const sections = cleanedContent.split(sectionRegex);
        const sectionTitles = cleanedContent.match(sectionRegex) || [];
        
        return (
          <div className="structured-analysis">
            {sections.map((sectionContent, index) => {
              if (index === 0 && !sectionContent.trim()) return null;
              
              const title = sectionTitles[index - 1] || `Section ${index}`;
              return (
                <AnalysisSection 
                  key={index} 
