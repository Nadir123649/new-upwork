...
const Chat = () => {
  ...
  useEffect(() => {
    ...
    const checkAuthInitiation = async () => {
      if (userId && isOnboarding) {
        try {
          const { data } = await axios.get(`http://localhost:5011/get_auth_initiation`, {
            params: { user_id: userId }
          });
          if (data.initiated) {
            setAuthInitiated(true);
            setAuthExpiresAt(new Date(data.expires_at));
          }
        } catch (error) {
    ...
    const checkAuthCompletion = async () => {
      if (userId && isOnboarding && authInitiated) {
        try {
          const { data } = await axios.get(`http://localhost:5011/check_auth_completion`, {
            params: { user_id: userId }
          });
          if (data.completed) {
            setMessages(prevMessages => [
              ...prevMessages,
              { role: 'assistant', content: 'Schwab authentication successful! ' + data.next_question }
            ]);
            // Update onboarding step here if necessary
    ...
    // Check for auth initiation every 30 seconds
    const initiationInterval = setInterval(checkAuthInitiation, 30000);

    // If auth is initiated, start checking for completion
    if (authInitiated) {
  ...
  }, [userId, isOnboarding, authInitiated, authExpiresAt]);
  ...
  useEffect(() => {
    ...
    if (authStatus === 'success') {
      ...
      if (nextStep) {
        dispatch(updateOnboardingStatus(nextStep));
        fetchNextOnboardingQuestion(nextStep);
      }
  ...
  const fetchNextOnboardingQuestion = async (step) => {
    try {
      const response = await axios.get(`http://localhost:5052/get_onboarding_question`, {
        params: { user_id: userId, step: step }
      });
      setMessages(prevMessages => [...prevMessages, { role: 'assistant', content: response.data.question }]);
    } catch (error) {
      console.error('Error fetching next onboarding question:', error);
    }
  };
