...
import { fetchOnboardingStatus, updateOnboardingResponse } from '../../store/actions/onboardingActions';
import './Chat.css';
...
const Chat = () => {
  ...
  useEffect(() => {
    if (onboardingQuestion && !onboardingCompleted) {
      setMessages(prevMessages => [...prevMessages, { role: 'assistant', content: onboardingQuestion }]);
    }
  }, [onboardingQuestion, onboardingCompleted]);
  ...
  useEffect(() => {
    const getTempUserId = async () => {
      if (!isAuthenticated && !localStorage.getItem('tempUserId')) {
        try {
          const { data } = await axios.get('http://localhost:5052/get_temp_user_id');
  ...
  const startNewConversation = async () => {
    try {
      ...
      const response = await axios.get('http://localhost:5052/get_conversation_id', {
        params: {
          user_id: userId,
    ...
    if (prompt.trim()) {
      ...
      try {
        if (!onboardingCompleted) {
          dispatch(updateOnboardingResponse(onboardingStep, prompt));
        } else {
      ...
      <form onSubmit={handleSubmit} className="chat-form">
        <div className="chat-form-inner">
          <textarea
            ...
            placeholder={!onboardingCompleted ? onboardingQuestion : "Enter new prompt ..."}
            required
            disabled={isLoading}
          ></textarea>
