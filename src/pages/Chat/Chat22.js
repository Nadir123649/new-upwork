...
import { fetchChatResponseCount } from '../../store/actions/userActions';
...
const Chat = () => {
  ...
  const [chatResponseLimit, setChatResponseLimit] = useState(null);
  ...
  useEffect(() => {
    const fetchChatResponseCount = async () => {
      if (effectiveUserId) {
        try {
          const response = await axios.get(`${API_ENDPOINTS.CHAT_RESPONSE_COUNT}/${effectiveUserId}`);
          setChatResponseCount(response.data.count);
          setChatResponseLimit(response.data.limit);
          setIsUnlimited(response.data.isUnlimited);
          
          if (!response.data.isUnlimited && response.data.count >= response.data.limit) {
          ...
          <div className="chat-form-actions">
            {!isUnlimited && chatResponseLimit !== null && (
              <div className="chat-limit-info">
                <p>{Math.max(0, chatResponseLimit - chatResponseCount)} {effectiveUserId < 4000 ? 'daily' : ''} messages remaining</p>
              </div>
            )}
