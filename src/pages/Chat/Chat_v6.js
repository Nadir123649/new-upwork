...
const Chat = () => {
  ...
  useEffect(() => {
    const fetchStatus = async () => {
      if (userId && isAuthorized) {
        try {
          ...
          } else if (!currentOnboardingStep) {
            // Only update if we don't already have an onboarding step
    ...
    // Remove the interval to prevent repetitive messages
    // const intervalId = setInterval(fetchStatus, 30000);
    // return () => clearInterval(intervalId);
  }, [userId, isAuthorized, currentOnboardingStep]);
          ...
          <div className="chat-main">
            <div className="chat-messages-wrapper" ref={chatMessagesWrapperRef}>
              <div className="chat-messages">
                {isAuthorized && currentOnboardingStep && messages.length === 0 && (
                  <div className="message assistant">
                    <div className="message-content">
                      <p>Welcome to the onboarding process. Please answer the following question to set up your account.</p>
                    </div>
                  </div>
                )}
                {(!isAuthorized || !currentOnboardingStep) && (
                  <div className="message assistant">
      ...
      <form onSubmit={handleSubmit} className="chat-form">
        <div className="chat-form-inner">
          <textarea
            ...
            placeholder={currentOnboardingStep ? currentOnboardingQuestion : "Enter new prompt ..."}
            required
            disabled={isLoading}
          ></textarea>
