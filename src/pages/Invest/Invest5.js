            ...
            {Array.from(xmlDoc.querySelectorAll('section')).map((section, index) => {
              ...
              return (
                <div key={index} className="analysis-section">
                  ...
                  {section.querySelector('products') && section.querySelectorAll('product').length > 0 && (
                        ...
                        <div key={subIndex} className="subsection">
                              ...
                              {Array.from(subsection.querySelectorAll('step')).map((step, stepIndex) => (
                                <div key={stepIndex} className="step">
                                    ...
                                    <div>
                                      <ul>
                                        {Array.from(step.querySelectorAll('metric')).map((metric, metricIndex) => (
                                          <li key={metricIndex}>
                                            <strong>{metric.querySelector('name')?.textContent}:</strong> {metric.querySelector('value')?.textContent} (weight: {metric.querySelector('weight')?.textContent})
                                          </li>
                                        ))}
                                      </ul>
                                      <div className="metric-explanations">
                                        ...
                                        <ul>
                                          {Array.from(step.querySelectorAll('metric')).map((metric, metricIndex) => (
                                            <li key={metricIndex}>
                                              <strong>{metric.querySelector('name')?.textContent}:</strong> {metric.querySelector('explanation')?.textContent}
                                            </li>
                                          ))}
                                        </ul>
                                      </div>
                                  ...
                                  {step.querySelector('score') && (
                                    <div>
                                      <p>Calculation: {step.querySelector('score calculation')?.textContent}</p>
                                      <p>Result: {step.querySelector('score result')?.textContent}</p>
                                    </div>
                                  )}
                                </div>
                              ))}
                          ...
                          {subsection.querySelector('finalScore') && (
                            <div className="final-score">
                              <h5>Final Score</h5>
                              <p>Calculation: {subsection.querySelector('finalScore calculation')?.textContent}</p>
                              <p>Value: {subsection.querySelector('finalScore value')?.textContent}</p>
                            </div>
                          )}
                          {subsection.querySelector('metrics') && !subsection.querySelector('steps') && (
                            <div className="metrics">
                                ...
                                {Array.from(subsection.querySelectorAll('metric')).map((metric, metricIndex) => (
                                  <li key={metricIndex}>
                                    ...
                                    {metric.querySelector('explanation') && (
                                      <p><em>Explanation: {metric.querySelector('explanation')?.textContent}</em></p>
                                    )}
                                  </li>
                                ))}
                              </ul>
                              {subsection.querySelector('score') && (
                                <div className="score">
                                  <p>Calculation: {subsection.querySelector('score calculation')?.textContent}</p>
                                  <p>Value: {subsection.querySelector('score value')?.textContent}</p>
                                </div>
                              )}
                            </div>
                          )}
                          {subsection.querySelector('universeStats') && (
                            <div className="universe-stats">
                              ...
                              <p>Minimum: {subsection.querySelector('universeStats minimum')?.textContent}</p>
                              <p>Maximum: {subsection.querySelector('universeStats maximum')?.textContent}</p>
                              <p>Average: {subsection.querySelector('universeStats average')?.textContent}</p>
                            </div>
                          )}
                          {subsection.querySelector('peerGroupStats') && (
                            <div className="peer-group-stats">
                              <h5>Peer Group Statistics</h5>
                              <p>Minimum: {subsection.querySelector('peerGroupStats minimum')?.textContent}</p>
                              <p>Maximum: {subsection.querySelector('peerGroupStats maximum')?.textContent}</p>
                              <p>Average: {subsection.querySelector('peerGroupStats average')?.textContent}</p>
                            </div>
                          )}
                          {subsection.querySelector('finalRanking') && (
                            <div className="final-ranking">
                              <h5>Final Rankings</h5>
                              <p>AI Rank (Universe): {subsection.querySelector('finalRanking aiRank universe')?.textContent}</p>
                              <p>AI Rank (Peer Group): {subsection.querySelector('finalRanking aiRank peerGroup')?.textContent}</p>
                              <p>Catholic Rank (Universe): {subsection.querySelector('finalRanking catholicRank universe')?.textContent}</p>
                              <p>Catholic Rank (Peer Group): {subsection.querySelector('finalRanking catholicRank peerGroup')?.textContent}</p>
                              <p>Overall Rank (Universe): {subsection.querySelector('finalRanking overallRank universe')?.textContent}</p>
                              <p>Overall Rank (Peer Group): {subsection.querySelector('finalRanking overallRank peerGroup')?.textContent}</p>
                            </div>
                          )}
                        </div>
                      ))}
