...
const Invest = () => {
  ...
  const renderPortfolioSummary = useCallback((xmlDoc) => {
    const summary = xmlDoc.querySelector('portfolio_summary');
    if (!summary) return null;

    return (
      <div className="portfolio-summary">
        <h2>Catholic Generative AI Efficiency Index Portfolio Summary</h2>
        <div className="companies">
          {Array.from(summary.querySelectorAll('company')).map((company, index) => (
            <div key={index} className="company">
              <h3>{company.querySelector('name').textContent} ({company.querySelector('symbol').textContent})</h3>
              <p><strong>Direction:</strong> {company.querySelector('direction').textContent}</p>
              <p><strong>Allocation:</strong> {company.querySelector('allocation').textContent}</p>
              <p><strong>AI Capability:</strong> {company.querySelector('ai_capability').textContent}</p>
              <p><strong>Catholic Alignment:</strong> {company.querySelector('catholic_alignment').textContent}</p>
              <p><strong>AI Rank:</strong> {company.querySelector('ai_rank > value').textContent} out of {company.querySelector('ai_rank > total').textContent}</p>
              <p><strong>Catholic Rank:</strong> {company.querySelector('catholic_rank > value').textContent} out of {company.querySelector('catholic_rank > total').textContent}</p>
            </div>
          ))}
        </div>
        <div className="conclusion">
          <h3>Conclusion</h3>
          <p>{summary.querySelector('conclusion').textContent}</p>
        </div>
      </div>
    );
  }, []);
  ...
  const renderOnboardingResponse = useCallback((parsedXML) => {
    ...
    return result;
  }, [currentOnboardingStep, handleBeginClick, handleSectorClick, userPoints, additionalCapital, totalInvestmentAmount, handleProceedClick, optimalPortfolio, portfolioSummary, newSymbol, handleRemoveStock, handleAddStock, handleProceedToSchwab, handleExecutePortfolio, replaceUrls, sectorPeerGroupMap]);
  ...
  const renderMessageContent = useCallback((content) => {
    ...
    if (typeof content === 'string') {
      ...
      if (cleanContent.includes('<portfolio_summary>')) {
        try {
          const xmlDoc = parseXML(cleanContent);
          if (xmlDoc.querySelector("parsererror")) {
            console.error("XML Parsing Error:", xmlDoc.querySelector("parsererror").textContent);
            result = <p>Error parsing portfolio summary. Please try again.</p>;
          } else {
            result = renderPortfolioSummary(xmlDoc);
          }
        } catch (error) {
          console.error("Error in XML parsing:", error);
          result = <p>Error processing portfolio summary. Please try again.</p>;
        }
      } else if (parsedXML && content === messages[messages.length - 1]?.content) {
    ...
    return result;
  }, [parsedXML, messages, renderPortfolioSummary, renderOnboardingResponse, replaceUrls]);
